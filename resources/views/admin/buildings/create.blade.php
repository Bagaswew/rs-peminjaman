    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="space-y-1">
                ...
                <h2 class="text-2xl font-extrabold tracking-tight text-slate-900">Tambah Gedung</h2>
                <p class="text-sm text-slate-500">Daftarkan unit gedung baru ke dalam sistem.</p>
            </div>
    ...
    <div class="mx-auto max-w-2xl">
        <form method="POST" action="{{ route('admin.buildings.store') }}"
            ...
            <div class="p-6 sm:p-8">
                @include('admin.buildings.partials.form', ['building' => null])
            </div>
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/60 px-6 py-4 sm:px-8">
                ...
                <button type="submit" class="rounded-xl bg-slate-900 px-6 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 hover:-translate-y-0.5 transition-all">Simpan Gedung</button>
            </div>
        </form>
    </div>
</x-app-layout>